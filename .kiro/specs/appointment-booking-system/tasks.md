...
- [x] 2. Database schema and migrations
  - [x] 2.1 Write Knex migration: `users`, `clients`, `staff_profiles`, `admin_profiles` tables with all constraints, indexes, and soft-delete columns (`archived_at`, `is_active`) exactly as specified in the design schema
    - _Requirements: 1.1, 1.5, 5.1, 6.1, 9.1_
  - [x] 2.2 Write Knex migration: `services` table with operating hours columns (`start_time`, `end_time`, `days_of_week`), capacity, soft-delete columns, and `created_by` FK
    - _Requirements: 7.1, 9.1_
  - [x] 2.3 Write Knex migration: `appointments` table with `tracking_number`, `personal_details` JSONB, `status` CHECK constraint, `archived_at`, and `processed_by` FK
...
- [ ] 10. Backend: REST API — authentication and profile routes
  - [ ] 10.1 Implement `POST /api/auth/register` (client self-registration), `POST /api/auth/login`, `POST /api/auth/logout`, `POST /api/auth/forgot-password`, `POST /api/auth/reset-password`
    - Apply `validatePasswordComplexity` on register and reset; call `AuditLogger.logUserAction` on login/logout
    - _Requirements: 1.1, 1.2, 1.4, 1.5, 1.6, 1.7_
  - [ ] 10.2 Implement `GET /api/profile`, `PUT /api/profile`, `POST /api/profile/change-password`, `DELETE /api/profile` (soft-deactivate)
    - Enforce RBAC: all roles may view/edit own profile; only clients may deactivate self
    - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5_
  - [ ] 10.3 Write JWT authentication middleware (`authenticateToken`) and role-guard middleware (`requireRole(...roles)`) used by all protected routes
    - _Requirements: 1.3_
...
- [ ] 15. Checkpoint — full API
  - Run all backend tests: `pnpm --filter backend test --run`
...
- [ ] 21. Checkpoint — full stack integration
  - Run all tests: `pnpm test --run` (root workspace command)
